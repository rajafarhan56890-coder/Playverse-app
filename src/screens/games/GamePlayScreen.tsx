...
import { completeLevelAndClaimCoins } from "../../services/games.service";
...
export default function GamePlayScreen() {
  ...
  const { gameId, levelNumber, coinsReward, gameName, engine } = route.params as {
    gameId: string;
    levelNumber: number;
    coinsReward: number;
    gameName: string;
  ...
  const [awardedCoins, setAwardedCoins] = useState(coinsReward);
  ...
  const handleClaimReward = async () => {
    ...
    try {
      const result = await completeLevelAndClaimCoins(gameId, levelNumber);
      if (result.success) {
        if (result.amountCredited) setAwardedCoins(result.amountCredited);
        setMessage(`🎉 +${result.amountCredited ?? coinsReward} coins awarded!`);
        setTimeout(() => {
          navigation.goBack();
        }, 2000);
      } else {
  ...
  const renderGame = () => {
    switch (engine) {
      ...
      default:
        return <Text style={{ color: colors.textPrimary }}>Game not found</Text>;
    }
  };
